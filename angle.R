...
t <- as.POSIXct("2020-02-15 11:46:44", tz="UTC") # 14:46:44 Halifax time
chi <- moonRotationAngleNEW(t, lon=lon, lat=lat)
par(mar=rep(0.5, 4))
ma <- moonAngle(t,lon=lon,lat=lat)
drawMoon(ma$phase, angle=chi-90)
ilfr <- ma$illuminatedFraction
mtext(paste0(format(t), ", chi=", round(chi,1), ", ilfr=", round(ilfr,3)), line=-1)
